...
def generate_business_insight(
    ...
    # Guard against division by zero (store with no recent sales)
    if recent_avg == 0:
        change_pct = 0.0
    else:
    ...
    if change_pct > 10:
        insights.append(
            f"Demand is expected to increase by approximately {change_pct:.1f}% "
            f"over the next two weeks."
        )
    elif change_pct < -10:
        insights.append(
            f"Demand is expected to decline by approximately {abs(change_pct):.1f}% "
            f"over the next two weeks."
        )
    else:
        insights.append(
    ...
    # Volatility insight — guard against zero avg_sales too
    if avg_sales > 0 and volatility > 0.5 * avg_sales:
        insights.append(
    ...
    # Recommendation
    insights.append(
        "Recommended action: plan inventory using weekly demand patterns "
        "and monitor sudden deviations closely."
    )
